...
function upload_files() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/framework/FW_PHP_OO_MVC_JQUERY/web2.0/media/'.$_FILES['file']['name'];//Cambiado avatar por file
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT']."/framework/FW_PHP_OO_MVC_JQUERY/web2.0/media/".$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    }
    ...
    if ($error == "") {
        if ($copiarFichero) {
            ...
            $upfile = '/framework/FW_PHP_OO_MVC_JQUERY/web2.0/media/'.$nombreFichero;
            return $return=array('result'=>true , 'error'=>$error,'data'=>$upfile);
        }
        if($_FILES['file']['error'] !== 0) { //Assignarem a l'us default-avatar
            $upfile = '/framework/FW_PHP_OO_MVC_JQUERY/web2.0/media/default-avatar.png';
            return $return=array('result'=>true,'error'=>$error,'data'=>$upfile);
        }
    }else{
        return $return=array('result'=>false,'error'=>$error,'data'=>"");
...
function remove_files(){
  ...
  //exit;
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/framework/FW_PHP_OO_MVC_JQUERY/web2.0/media/'.$_SESSION['nombreFichero'])){
		unlink($_SERVER['DOCUMENT_ROOT'].'/framework/FW_PHP_OO_MVC_JQUERY/web2.0/media/'.$_SESSION['nombreFichero']);
		return true;
	}else{
		return false;
	}
